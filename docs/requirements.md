...
| v0.26 | 2026-06-16 | UI 刷新(self-host CSS: topbar+card)。セッション: ブラウザ終了で失効・サーバー側上限12h。結果画面で MET/未該当/suppressed をカラータグ化＋MET行強調、分類を色付きバッジ(Pathogenic赤/Benign緑/VUS灰)。MFA 免除フラグ(User.mfa_exempt, レビュー用) 追加: 強制ミドルウェア/mfa_setup でスキップ、admin で administrator が設定 |
